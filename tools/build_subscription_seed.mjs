...
const statements = [
  ...
  `CREATE TABLE IF NOT EXISTS subscription_products (id TEXT PRIMARY KEY, set_id TEXT NOT NULL, brand TEXT NOT NULL, category TEXT NOT NULL, source_category TEXT NOT NULL DEFAULT '', model TEXT NOT NULL, name TEXT NOT NULL, monthly_fee_72 INTEGER NOT NULL, care_type TEXT DEFAULT '', care_detail TEXT DEFAULT '', visit_cycle TEXT DEFAULT '', image_url TEXT DEFAULT '', sort_order INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL);`,
...
payload.items.forEach((item, index) => {
  statements.push(
    `INSERT INTO subscription_products (id, set_id, brand, category, source_category, model, name, monthly_fee_72, care_type, care_detail, visit_cycle, image_url, sort_order, created_at) VALUES (` +
    `${quote(`${setId}-${String(index + 1).padStart(4, "0")}`)}, ${quote(setId)}, ${quote(item.brand)}, ${quote(item.category)}, ${quote(item.sourceCategory)}, ${quote(item.model)}, ${quote(item.name)}, ${Number(item.monthlyFee72)}, ${quote(item.careType)}, ${quote(item.careDetail)}, ${quote(item.visitCycle)}, ${quote(item.imageUrl)}, ${index}, ${quote(now)});`
  );
});
